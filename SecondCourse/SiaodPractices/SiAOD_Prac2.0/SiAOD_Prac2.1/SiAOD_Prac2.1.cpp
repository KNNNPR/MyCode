...
table* binarSearch(std::vector<table>& a, int i, int key,  int delta)
{
    /*if (delta == 0) return(nullptr);*/
    if (i < 0) return(nullptr);
    if (a[i].numberTicket == key) return(&a[i]);
    if (a[i].numberTicket < key) return(binarSearch(a, i + delta / 2 + 1, key, delta / 2));
    if (a[i].numberTicket > key) return(binarSearch(a, i - delta / 2 - 1, key, delta / 2));
}
void task3()
{
    ...
    std::cin >> keyO;
    table* result = binarSearch(testVec, testVec.size()-1, keyO, testVec.size() - 1);
