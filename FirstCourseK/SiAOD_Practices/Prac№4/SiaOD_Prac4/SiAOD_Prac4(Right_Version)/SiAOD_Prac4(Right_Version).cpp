...
bool matchDate(Date dateOfDep, int d, int m, int y)
{
	if (dateOfDep.day != d || dateOfDep.month != m || dateOfDep.year != y)
	{
		return (false);
	}
	return (true);
	...
	for (int i = 0; i < wagonLength; i++)
	{
		for (int j = 0; j < length; j++)
		{
			if (List[j].Wagon == wagons[i])
			{
				...
				std::cout << "Time " << List[j].dateOfDep.day << ':' << List[j].dateOfDep.month << ':' << List[j].dateOfDep.year << std::endl;
			}
		}
		std::cout << std::endl;
	}
